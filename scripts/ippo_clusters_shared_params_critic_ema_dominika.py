...
class SharedPPO(BaseLearningModel):
    ...
    def learn(self):
        ...
        all_rewards = [transition[3] for transition in self.memory]
        current_mean_reward = sum(all_rewards) / len(all_rewards)
        if self.ema_baseline is None:
            self.ema_baseline = current_mean_reward
        else:
            self.ema_baseline = self.ema_alpha * self.ema_baseline + (1 - self.ema_alpha) * current_mean_reward
        ...
        for _ in range(self.num_epochs):
            ...
            centered_rewards = rewards_tensor - self.ema_baseline
            
            advantage_raw = centered_rewards - values.detach()
            ...
            critic_loss = nn.MSELoss()(values, centered_rewards)
...
if __name__ == "__main__":
    ...
    for episode in range(training_eps):
        ...
        episode_losses = shared_model.loss[-1:] if len(shared_model.loss) > 0 else []
        ...
        if shared_model.ema_baseline is not None:
            metrics["train/ema_baseline"] = shared_model.ema_baseline
        wandb.log(metrics)
